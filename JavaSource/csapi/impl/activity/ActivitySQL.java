...
//		sb.append("   LEFT OUTER JOIN Q on A.ID = Q.ACTIVITY_ID ");
	...
	public static String inheritExpiration(int projectid) {
		...
		sb.append(" SET ");
		sb.append(" 	EXP_DATE = ");
		sb.append(" case when exp_date > ( SELECT CASE WHEN APPROVED_REVIEW_PERMITDAYS_TILL_EXPIRED > 0 THEN getDate() + APPROVED_REVIEW_PERMITDAYS_TILL_EXPIRED ELSE getDate() END AS EXP_DATE "); 
		sb.append(" FROM LKUP_ACT_TYPE WHERE LKUP_ACT_TYPE.ID = LKUP_ACT_TYPE_ID ) ");
		sb.append(" then  exp_date ");
		sb.append(" else ( SELECT CASE WHEN APPROVED_REVIEW_PERMITDAYS_TILL_EXPIRED > 0 THEN getDate() + APPROVED_REVIEW_PERMITDAYS_TILL_EXPIRED ELSE getDate() END AS EXP_DATE "); 
		sb.append(" FROM LKUP_ACT_TYPE WHERE LKUP_ACT_TYPE.ID = LKUP_ACT_TYPE_ID ) "); 
		sb.append(" end ");
		sb.append(" WHERE ");
		sb.append(" 	INHERIT = 'Y' ");
