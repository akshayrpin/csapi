...
public class ProjectinfoImpl {
	...
	public static String save(HttpServletRequest request, HttpServletResponse response, String json) {
		...
		try {
			...
			r = ProjectinfoAgent.add(vo.getType(), vo.getTypeid(), Operator.toInt(vo.getId()), m.getString("LKUP_PROJECTINFO_PROJECT_ID"), m.getString("LKUP_PROJECTINFO_BUILDINGTYPE_ID"), m.getString("LKUP_PROJECTINFO_BUILDINGUSE_ID"), m.getString("LKUP_PROJECTINFO_USE_ID"), m.getString("OTHER_PROJECT_TYPE"), m.getString("OTHER_BUILDING_TYPE"), m.getString("OTHER_BUILDING_USE"), m.getString("OTHER_USE_EXISTING"), m.getString("OTHER_USE_PROPOSED"),u.getId(), u.getIp());
			s = mapper.writeValueAsString(r);
