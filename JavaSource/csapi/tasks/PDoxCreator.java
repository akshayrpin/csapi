...
public class PDoxCreator {
	...
	public static String path =Config.fullcontexturl()+"/jsp/tasks/pdoxcreator.jsp";
	...
	public static MapSet getOptions(Cartographer map){
		...
		sb.append(" SELECT TOP 1 ID FROM TASKS_PDOX_CREATOR  WHERE ACTIVE='Y' AND CREATED_BY = ").append(m.getString("CREATED_BY")).append(" AND  ( TEMPLATE_ID= ").append(m.getString("TEMPLATE_ID")).append(" OR CURRENT_STATUS_ID = '").append(m.getString("CURRENT_STATUS_ID")).append("' ) ORDER BY ID DESC ");
	...
	public TaskVO getSettings(){
		Sage db = new Sage();
		String command = "select * from TASKS_PDOX_CREATOR where ID ="+TASK.getTaskid();
		db.query(command);
	...
	public static ArrayList<HashMap<String,String>> getListtoProcess(TaskVO t){
		...
		if(Operator.hasValue(command)){
			...
			db.query(command);
			if(db.next()){
				if((db.getDouble("FEE_AMOUNT")>0.00 && db.getDouble("BALANCE_DUE")==0.00) || (db.getDouble("FEE_AMOUNT")==0.00 && db.getDouble("BALANCE_DUE")==0.00)){
					if(Operator.hasValue(db.getString("LIST_PEOPLE_PRIMARY"))){
						
						
						
						boolean r = PDoxAPI.createProject(db.getString("ACT_NBR"),db.getString("DESCRIPTION"),db.getString("TYPE"),db.getString("LIST_PEOPLE_PRIMARY"));
						
						if(r){
							HashMap<String,String> id = new HashMap<String,String>();
							id.put("ID", db.getString("ID"));
							id.put("activity_act_nbr", db.getString("ACT_NBR"));
							id.put("activity_type", db.getString("TYPE"));
	...
	public static String buildQuery(TaskVO t){
		...
		sb.append(" select  A.ID,A.ACT_NBR,A.DESCRIPTION, CASE WHEN sum(FEE_AMOUNT) is null then 0.00 else sum(FEE_AMOUNT) END as FEE_AMOUNT , CASE WHEN sum(FEE_PAID) is null then 0.00 else sum(FEE_PAID) END as FEE_PAID , CASE WHEN sum(BALANCE_DUE) is null then 0.00 else sum(BALANCE_DUE) END as BALANCE_DUE,LAT.TYPE AS TYPE ");
		...
		sb.append(" LEFT OUTER JOIN STATEMENT_DETAIL SD on RAS.STATEMENT_ID= SD.STATEMENT_ID AND SD.ACTIVE='Y' ");
		...
		//sb.append(" AND SD.UPDATED_DATE  BETWEEN CAST(getDate() -").append(5).append(" as date)  AND CAST(getDate() +1").append(1).append(" as date)  ");
		...
		sb.append(" group by A.ID ,A.ACT_NBR,LAT.TYPE,A.DESCRIPTION ");
		return sb.toString();
	}
