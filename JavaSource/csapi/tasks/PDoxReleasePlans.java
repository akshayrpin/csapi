...
public class PDoxReleasePlans {
	...
	public static String path =Config.fullcontexturl()+"/jsp/tasks/pdoxreleaseplans.jsp";
	...
	public static MapSet getOptions(Cartographer map){
		...
		sb.append(" SELECT TOP 1 ID FROM TASKS_PDOX_RELEASE_PLAN  WHERE ACTIVE='Y' AND CREATED_BY = ").append(m.getString("CREATED_BY")).append(" AND  ( TEMPLATE_ID= ").append(m.getString("TEMPLATE_ID")).append(" OR STATUS_CHANGE_ID = '").append(m.getString("STATUS_CHANGE_ID")).append("' ) ORDER BY ID DESC ");
	...
	public TaskVO getSettings(){
		Sage db = new Sage();
		String command = "select * from TASKS_PDOX_RELEASE_PLAN where ID ="+TASK.getTaskid();
		db.query(command);
			...
			if(db.next()){
				if(db.getDouble("BALANCE_DUE")==0.00){
					if(Operator.hasValue(db.getString("LIST_PEOPLE_PRIMARY"))){
						...
						boolean r = PDoxAPI.releasePlans(db.getString("ACT_NBR"));
						r = true;
						if(r){
	...
	public static String buildQuery(TaskVO t){
		...
		sb.append(" group by A.ID ,A.ACT_NBR,LAT.DESCRIPTION,A.DESCRIPTION ");
		return sb.toString();
	}
	...
	public static boolean updateStatus(int actid,TaskVO t){
		...
		if(statusId>0){
			...
			sb.append(" WHERE  LKUP_ACT_STATUS_ID=").append(t.getTaskdetails().get("CURRENT_STATUS_ID")).append(" AND  ID = ").append(actid);
