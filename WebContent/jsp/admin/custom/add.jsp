		...
		<div id="csuimain">
			...
			<div class="csuicontent">
				<table cellpadding="10" cellspacing="0" border="0" width="100%">
					<tr>
						<td align="left" id="title">FIELD GROUP</td>
				...
				<form id="csform" class="form" ajax="no" action="add.jsp" method="post">
					...
					<table class="csui_title">
						<tr>
							<td class="csui_title">FIELD GROUP</td>
						...
						<table class="csui" colnum="2" type="default">
							...
							<tr>
								<td class="csui_label" colnum="2" alert="">ISMULTI</td>
								<td class="csui" colnum="2" type="boolean" itype="boolean" alert=""><div><input name="MULTI" type="checkbox" itype="boolean" <%if(v.getString("MULTI").equals("Y")){ %>checked <% }%> ></div></td>
			...
			<div class="csui_buttons">
				...
				<table class="csui tablesorter" type="horizontal">
						...
						<tr>
							...
							<td class="csui_header">ITYPE</td>
							<td class="csui_header">REQ</td>
							<td class="csui_header">MAX CHAR</td>
							<td class="csui_header">IDX</td>
						...
						%>
							<tr id="tr_<%=r.getString("ID") %>">
								...
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("ITYPE") %></td>
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("REQUIRED") %></td>
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("MAX_CHAR") %></td>
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("IDX") %></td>
							    ...
							    <td class="csui" width="1%">
									<a target="lightbox-iframe" href="field/copy.jsp?_fieldId=<%=r.getString("ID")%>&_id=<%=id%>&ID=<%=r.getString("ID") %>&_type=<%=type%>&_typeid=<%=typeid%>&_groupid=<%=editId%>&_fieldname=<%=r.getString("NAME") %>&process=edit" title="Copy-Field"><img src="/cs/images/icons/copy.png" border="0"></a>
								</td>
								<td class="csui" width="1%">
								<%
								String color="black";
								  if(r.getInt("CHOICE_COUNT")>0) {
									  color="color";
								  }  
								%>
								
									<a target="lightbox-iframe" href="field/choices/list.jsp?_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>&ID=<%=r.getString("ID") %>&_groupid=<%=editId%>&_groupname=<%=v.getString("GROUP_NAME") %>-<%=r.getString("NAME") %>&process=edit" title="Manage Choices"><img src="/cs/images/icons/controls/<%=color %>/choices.png" border="0"></a>
								
								</td>
								<td class="csui" width="1%">
									<a target="lightbox-iframe" href="field/add.jsp?_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>&ID=<%=r.getString("ID") %>&_groupid=<%=editId%>&_groupname=<%=v.getString("GROUP_NAME") %>&process=edit" title="Edit-Field"><img src="/cs/images/icons/controls/black/edit.png" border="0"></a>
								</td>
								<td class="csui" width="1%">
