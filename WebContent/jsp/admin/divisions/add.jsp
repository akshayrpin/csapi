...
<%
	...
	String table = Table.LKUPDIVISIONSTYPETABLE;
	String alert="";
	...
	if(editId>0){
		...
		fields = AdminAgent.getList(AdminMap.getDivisionsFields(editId));
	}
	
	boolean result = false;
	 if(map.equalsIgnoreCase("_action","SAVE")){
		 if(editId>0){
			 result = AdminAgent.updateType(AdminMap.getDivisionsType(map),table);
		 }else {
			 result = AdminAgent.saveType(AdminMap.getDivisionsType(map),table);
		 }
	 ...
	 JSONArray groups = AdminAgent.getLookup("LKUP_DIVISIONS_GROUP");
		...
		$(document).ready(function() {
			...
			<% if(editId>0){%>
				...
				$('input:radio[name=online]:checked');
				 ...
				 $('#LKUP_DIVISIONS_GROUP_ID').val(<%=v.getString("LKUP_DIVISIONS_GROUP_ID")%>);
				 $('#LKUP_DIVISIONS_GROUP_ID').trigger('chosen:updated');
			...
			swal({  
				...
				function(isConfirm){   
					if (isConfirm) {     
						$.ajax({
				   			  type: "POST",
				   			  url: "<%=Config.fullcontexturl()%>/jsp/admin/divisions/action.jsp?_action="+method,
				   			  dataType: 'json',		  
				   			  data: { 
				   				  _type : "<%=type%>",
				   				  _typeid : <%=typeid%>,
		...
		<div id="csuimain">
			...
			<div class="csuicontent">
				<table cellpadding="10" cellspacing="0" border="0" width="100%">
					<tr>
						<td align="left" id="title">DIVISION TYPE</td>
				...
				<form id="csform" class="form" ajax="no" action="add.jsp" method="post">
					...
					<table class="csui_title">
						<tr>
							<td class="csui_title">DIVISION TYPE</td>
						...
						<table class="csui" colnum="2" type="default">
							<tr>
								...
								<td class="csui" colnum="2" type="String" itype="text" alert=""><input name="TYPE" type="text" itype="text" value="<%=v.getString("TYPE")%>" valrequired="true"  maxchar="25"></td>
								<td class="csui_label" colnum="2" alert="">DESCRIPTION</td>
								<td class="csui" colnum="2" type="String" itype="text" alert=""><input name="DESCRIPTION" type="text" itype="text" value="<%=v.getString("DESCRIPTION") %>" valrequired="true" maxchar="25"></td>
							</tr>
							<tr>
								<td class="csui_label" colnum="2" alert="">REQUIRED</td>
								<td class="csui" colnum="2" type="boolean" itype="boolean" alert="" ><div><input name="REQUIRED" type="checkbox" itype="boolean" <%if(v.getString("REQUIRED").equals("Y")){ %>checked <% }%> ></div></td>
							...
							<tr>
								...
								<select  name="LKUP_DIVISIONS_GROUP_ID" id="LKUP_DIVISIONS_GROUP_ID" itype="String" val="" _ent="lso" valrequired="false"><option value=""></option>
									<%for(int i=0;i<groups.length();i++){ %>	<option value="<%=groups.getJSONObject(i).getInt("ID")%>"><%=groups.getJSONObject(i).getString("TEXT")%></option>		<%}%>
								</select>
			...
			<div class="csui_buttons">
				...
				<table class="csui_title">
						<tr>
							<td class="csui_title">DIVISIONS</td>
							<td class="csui_title">
								<a target="lightbox-iframe" href="field/add.jsp?_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>&_groupid=<%=editId%>&_groupname=<%=v.getString("DESCRIPTION") %>" ><img src="/cs/images/icons/controls/white/add.png" border="0"></a>
							</td>
				...
				<table class="csui tablesorter" type="horizontal">
						...
						<tr>
							...
							<td class="csui_header">DIVISION</td>
						...
						%>
							<tr id="tr_<%=r.getString("ID") %>">
								...
								<td class="csui" style="cursor:pointer;" rel="<%=r.getString("ID") %>" ><%=r.getString("DIVISION") %>
								 <input type="hidden" id="refname_<%=r.getString("ID") %>" name="refname_<%=r.getString("ID") %>" value="<%=r.getString("DIVISION_TYPE") %>">
								</td>
								...
								<td class="csui" width="1%">
									<a target="lightbox-iframe" href="field/add.jsp?_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>&ID=<%=r.getString("ID") %>&_groupid=<%=editId%>&_groupname=<%=v.getString("DESCRIPTION") %>&process=edit" ><img src="/cs/images/icons/controls/black/edit.png" border="0"></a>
								</td>
								<td class="csui" width="1%">
