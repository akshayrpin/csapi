...
<%
	...
	String FEE_GROUP_ID = map.getString("FEE_GROUP_ID","0");
	...
	String table = Table.LKUPDIVISIONSTABLE; 
	...
	String sortfield= map.getString("SORT_FIELD","ORDR");
	...
	//	urlsort.append("/admin/formmgr/formEntries.jsp?SEARCH=SEARCH&FORM_ID=").append(formId).append("&CATEGORY_ID=").append(categoryId).append("&MAX=").append(maxrows).append("&Q=").append(q).append("&searchfields=").append(searchfields).append("&displayfields=").append(displayfields);
	 ...
	 JSONArray types = AdminAgent.getLookup("LKUP_DIVISIONS_TYPE");
...
<head>
	...
	<script>
		$(document).ready(function() {
				 ...
				 $('#SQ').val(<%=map.getString("SQ")%>);
				 $('#SQ').trigger('chosen:updated');
	...
	function showselectorui(id,output){
			...
			//output = JSON.stringify(output);
	 		...
	 		c += '<td class="csui_header pymnt" width="1%">&nbsp;</td>';
	 		c += '<td class="csui_header pymnt" width="1%">&nbsp;</td>';
			c += '<td class="csui_header pymnt" width="1%" nowrap>DIVISIONS</td>';
			c += '<td class="csui_header pymnt" width="1%" nowrap>DESCRIPTION</td>';
			...
			//c += '<table class="sortable">';
	 		...
	 		$.each(output, function(k,v) {
	 				...
	 				c+= '<td class="csui pymnt" width="1%"><input type="checkbox" name="selector" id="selector" class="selector csui" value="'+v.ID+'"></td>';
	 				c += '<td class="csui pymnt" type="String"  itype="String" width="1%" nowrap>'+t+++'</td>';
			 		c += '<td class="csui pymnt" type="String"  itype="String" width="1%" nowrap>'+v.DIVISIONS+'</td>';
			 		c += '<td class="csui pymnt" type="String" itype="String" width="1%" nowrap>'+v.DESCRIPTION+'</td>';
	...
	function appendrefs(id){
		...
		$.ajax({
 			  ...
 			  data: { 
 				  ...
 				  LKUP_DIVISIONS_ID : id,
 				  FEE_GROUP_ID : <%=FEE_GROUP_ID%>,
 				  LIBRARY_GROUP_ID : <%=map.getString("groupId","0")%>
 			      //mode : mode
 			    },
 			    success: function(output) {
					...
					$.ajax({
			   			  ...
			   			  data: { 
			   				  ...
			   				  _id : <%=id%>,
			   				   ...
			   				   //REF_ID : id,
			   				   FEE_GROUP_ID : <%=FEE_GROUP_ID%>,
			   				   LKUP_DIVISIONS_ID : fieldgroupsid
			   			      //mode : mode
			   			    },
			   			    success: function(output) {
	...
	<div id="csuibody">
		<div id="csuimain">
			...
			<br/><br/>
				...
				<form id="csform" action="clist.jsp" method="post">
				...
				<input type="hidden" name="SEARCH_FIELDS" value="LKUP_DIVISIONS_TYPE_ID">
				...
				<table align="right" style="width:100%">
					...
					<tr>
						<td class="search"  type="String" itype="String" alert="" align="left">
							
							<select  name="SQ" id="SQ" itype="String" val="" _ent="lso" valrequired="true" onchange="submit();"><option value=""></option>
						...
						%>
							<tr id="tr_<%=r.getString("ID") %>">
								...
								<td class="csui" style="cursor:pointer;" rel="<%=r.getString("ID") %>" ><%=r.getString("DIVISION") %>
								 <input type="hidden" id="refname_<%=r.getString("ID") %>" name="refname_<%=r.getString("ID") %>" value="<%=r.getString("DIVISION") %>">
								...
								%>
									...
									<table class="csui" type="horizontal" id="cr_<%=r.getString("ID")%>" style="cursor:pointer;" title="Drag to sort">
										<tr>
											...
											<td class="csui">
												<%=r.getString("TYPE")%>-<%=r.getString("DIVISION")%>
											</td>
