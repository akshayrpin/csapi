...
<%
	...
	if(connector){
	 ...
	 if(map.equalsIgnoreCase("SEARCH","SEARCH")){
		a =  AdminAgent.getListWithAdditional(table,start,end,sortfield,order,map.getString("SQ"),map.getString("SEARCH_FIELDS"),",G.GROUP_NAME","LEFT OUTER JOIN LKUP_DIVISIONS_GROUP G on A.LKUP_DIVISIONS_GROUP_ID=G.ID AND G.ACTIVE='Y'","");
		//records =5;
	...
	//	urlsort.append("/admin/formmgr/formEntries.jsp?SEARCH=SEARCH&FORM_ID=").append(formId).append("&CATEGORY_ID=").append(categoryId).append("&MAX=").append(maxrows).append("&Q=").append(q).append("&searchfields=").append(searchfields).append("&displayfields=").append(displayfields);
		...
		a =  AdminAgent.getListWithAdditional(table,start,end,sortfield,order,"","",",G.GROUP_NAME","LEFT OUTER JOIN LKUP_DIVISIONS_GROUP G on A.LKUP_DIVISIONS_GROUP_ID=G.ID AND G.ACTIVE='Y'","");
		//records =11;
		...
		$(document).ready(function() {
				...
				<%if(Operator.hasValue(connectids)){ %>
				 ...
				 <%for(int i=0;i<c.size();i++){
					...
					%>
					var id = "<%= r.getString("LKUP_DIVISIONS_TYPE_ID")%>";
					$('#tr_'+id).hide();
					
					<%}%>
				 
				 $('.csui').click(function() {
						var id = 0;//$(this).attr('rel');
						//alert(id);
						if(id>0){
							appendrefs(id);
						}
...
//	 		c += '<td class="csui_header pymnt" width="1%">&nbsp;</td>';
			...
			c += '<td class="csui_header pymnt">DESCRIPTION</td>';
...
//	 				c+= '<td class="csui pymnt" width="1%">&nbsp;</td>';
	 				c += '<td class="csui pymnt" type="String"  itype="String" width="1%" nowrap>'+t+++'</td>';
			 		c += '<td class="csui pymnt" type="String"  itype="String" width="1%" nowrap>'+v.DIVISION+'</td>';
			 		c += '<td class="csui pymnt" type="String" itype="String">'+v.DESCRIPTION+'</td>';
		...
		swal({  
			...
			function(isConfirm){   
				if (isConfirm) {     
					$.ajax({
			   			  type: "POST",
			   			  url: "<%=Config.fullcontexturl()%>/jsp/admin/territory/action.jsp?_action="+method,
			   			  dataType: 'json',		  
			   			  data: { 
			   				// feesjson : type,
			   			      _grptype : "finance",
