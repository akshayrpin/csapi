	...
	function showselectorui(id,output){
			...
			//c += '<table class="sortable">';
	 		...
	 		$.each(output, function(k,v) {
			 		...
			 		c += '<td class="csui pymnt" type="String" itype="String" width="1%" nowrap>'+v.KEY_CODE+'</td>';
			 		...
			 		var uc = "cplist.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>&process=edit&ID=";
			 		uc +=v.ID; 
			 		uc +="&groupId="+id; 
			 		uc +="&feeId="+v.FEE_ID; 
			 		uc +="&REQ="+v.REQ; 
			 		...
			 		var tinn = "../divisions/clist.jsp?_ent=<%=ent%>&_id=10&_type=<%=type%>&_typeid=<%=typeid%>&view=connector&t=inn&connecttype=feeinn&ID=";
			 		tinn +=v.FEE_ID; 
			 		tinn +="&connectids="+v.FEE_ID; 
			 		tinn +="&FEE_GROUP_ID="+id; 
			 		var tout = "../divisions/clist.jsp?_ent=<%=ent%>&_id=10&_type=<%=type%>&_typeid=<%=typeid%>&view=connector&t=inn&connecttype=feeout&ID=";
			 		tout +=v.FEE_ID; 
			 		tout +="&connectids="+v.FEE_ID;
			 		tout +="&FEE_GROUP_ID="+id;
			 		var associatecolor = "black"; 	if(v.MULTI>1){ 	associatecolor ="color";	}
			 		var inncolor = "black"; 	if(v.FINN_ID>0){ 		inncolor ="color"; 		}
			 		var outcolor = "black";		if(v.FOUT_ID>0){		outcolor ="color";		}
			 		
			 		c += '<td class="csui pymnt" type="String" itype="String" align="center" width="1%" nowrap><a title="Copy '+v.NAME+'" target="lightbox-iframe" href="'+uc+'" ><img src="/cs/images/icons/controls/black/copy.png" border="0"></a></td>';
			 		c += '<td class="csui pymnt" type="String" itype="String" align="center" width="1%" nowrap><a title="Associate '+v.NAME+'" target="lightbox-iframe" href="'+u+'" ><img src="/cs/images/icons/controls/'+associatecolor+'/copy.png" border="0"></a></td>';
				   ...
				   <table class="csui_title" alert="warning">
						...
						%>
							<tr id="tr_<%=r.getString("ID") %>">
								...
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("START_DATE") %></td>
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("EXPIRATION_DATE") %></td>
