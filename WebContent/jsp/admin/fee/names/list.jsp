...
<%
	...
	if(connector){
	 ...
	 if(map.equalsIgnoreCase("SEARCH","SEARCH")){
		if(!connector){
			...
			a = AdminAgent.getList(AdminMap.getFeeNames(map.getString("SQ")));
		}
		//records =5;
	...
	//	url.append("clist.jsp?_ent=admin&_id=").append(id).append("&_type=").append(type).append("&_typeid=").append(typeid).append("&ID=").append(ID).append("&SQ=").append(map.getString("SQ")).append("&MAX=").append(maxrows).append("&SORT_FIELD=").append(sortfield).append("&ORDER=").append(order);
		...
		if(!connector){
			...
			a = AdminAgent.getList(AdminMap.getFeeNames(""));
		}
		//records =11;
	...
	<script>
		$(document).ready(function() {
			 ...
			 $(".selector").click(function(){
				 ...
				 window.parent.$("#feename").html(name);
				 window.parent.$("#FEE_ID").val(v);
	...
	<div id="csuibody">
		<div id="csuimain">
			...
			<br/><br/>
				...
				<form id="csform" ajax="no" class="form" action="list.jsp" method="post">
