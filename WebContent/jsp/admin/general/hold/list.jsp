...
<%
	...
	//	urlsort.append("/admin/formmgr/formEntries.jsp?SEARCH=SEARCH&FORM_ID=").append(formId).append("&CATEGORY_ID=").append(categoryId).append("&MAX=").append(maxrows).append("&Q=").append(q).append("&searchfields=").append(searchfields).append("&displayfields=").append(displayfields);
	 ...
	 int totalpages = Operator.getTotalPages(records,maxrows);
		...
		if(connector){
			MapSet m = AdminMap.getHoldsTypeRef(map);
		...
		$(document).ready(function() {
				...
				<%if(Operator.hasValue(connectids)){ %>
				 ...
				 <%for(int i=0;i<c.size();i++){
					...
					%>
					var id = "<%= r.getString("LKUP_HOLDS_TYPE_ID")%>";
					$('#tr_'+id).hide();
					...
					$.ajax({
			   			  ...
			   			  data: { 
			   				  ...
			   				  _id : <%=id%>,
			   				   ...
			   				   connectids : "<%=connectids%>",
			   				   //REF_ID : id,
			   				   LKUP_HOLDS_TYPE_ID : fieldgroupsid
			   			      //mode : mode
			   			    },
			   			    success: function(output) {
		...
		<div id="csuimain">
			...
			<br/><br/>
				...
				<table align="right">
					...
					<tr>
						<td class="search"  type="String" itype="String" alert="" align="right">
							<select  name="LOCATION" id="LOCATION" itype="String" val="" _ent="activity" valrequired="true" >
						...
						<tr>
							<td class="csui">
								...
								%>
									...
									<table class="csui" type="horizontal" id="cr_<%=r.getString("LKUP_HOLDS_TYPE_ID")%>" style="cursor:pointer;" title="Drag to sort">
										<tr>
											<td class="csui" width="1%">
												<img src="/cs/images/icons/controls/black/delete.png" border="0" onclick="deleteref(<%=r.getString("LKUP_HOLDS_TYPE_ID") %>);" >
											</td>
											<td class="csui">
