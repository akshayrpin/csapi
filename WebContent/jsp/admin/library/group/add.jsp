			...
			swal({  
				...
				function(isConfirm){   
					if (isConfirm) {     
						$.ajax({
				   			  type: "POST",
				   			  url: "<%=Config.fullcontexturl()%>/jsp/admin/library/group/action.jsp?_action="+method,
				   			  dataType: 'json',		  
				   			  data: { 
				   				// feesjson : type,
			...
			<div class="csui_buttons">
				...
				<table class="csui_title">
						<tr>
							...
							<td class="csui_title">
									 <a target="lightbox-iframe" href="/csapi/jsp/admin/library/field/add.jsp?_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>&_groupid=<%=editId%>&_groupname=<%=v.getString("GROUP_NAME")%>" ><img src="/cs/images/icons/controls/white/add.png" border="0"></a> 
				...
				<table class="csui tablesorter" type="horizontal">
						...
						<tr>
							...
							<td class="csui_header">DIVISION IN</td>
							<td class="csui_header">DIVISION NOT-IN</td>
						...
						<%for(int i=0;i<fields.size();i++){
							...
							String inncolor = "black"; 	if(r.getInt("LINN_ID")>0){ 		inncolor ="color"; 		}
							String outcolor = "black";	if(r.getInt("LOUT_ID")>0){		outcolor ="color";		}
							...
							String divisioninn = "/csapi/jsp/admin/divisions/clist.jsp?_ent=permit&_id=10&_type="+type+"&_typeid="+typeid+"&view=connector&t=inn&connecttype=libraryinn&ID="+r.getString("ID")+"&groupId="+editId+"&LIBRARY_GROUP_ID="+editId+"&libraryId="+r.getString("ID")+"&connectids="+r.getString("ID")+"";
							String divisionout = "/csapi/jsp/admin/divisions/clist.jsp?_ent=permit&_id=10&_type="+type+"&_typeid="+typeid+"&view=connector&t=inn&connecttype=libraryout&ID="+r.getString("ID")+"&groupId="+editId+"&LIBRARY_GROUP_ID="+editId+"&libraryId="+r.getString("ID")+"&connectids="+r.getString("ID")+"";
							
							%>
							<tr id="tr_<%=r.getString("REF_LIBRARY_GROUP_ID") %>">
								...
								<td class="csui" style="cursor:pointer;"   rel="<%=r.getString("ID") %>" >
								<a title="Divisions -in" target="lightbox-iframe" href="<%=divisioninn %>" ><img src="/cs/images/icons/controls/<%=inncolor%>/divisionsinclude.png" border="0"></a>
								</td>
								<%-- <td class="csui scrollselector" stid="<%=r.getString("ID") %>" title="Show Fields"> --%>
								<td class="csui" style="cursor:pointer;"   rel="<%=r.getString("ID") %>" >
								 <a title="Divisions -other" target="lightbox-iframe" href="<%=divisionout %>" ><img src="/cs/images/icons/controls/<%=outcolor%>/divisionsexclude.png" border="0"></a>
								 <input type="hidden" id="show_bottom_<%=r.getString("ID") %>" name="show_bottom_<%=r.getString("ID") %>" value="0">
								</td>
									
								
								<td class="csui" width="1%">
									<a target="lightbox-iframe" href="/csapi/jsp/admin/library/field/add.jsp?_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>&ID=<%=r.getString("ID") %>&_groupid=<%=editId%>&_groupname=<%=v.getString("GROUP_NAME") %>&process=edit" ><img src="/cs/images/icons/controls/black/edit.png" border="0"></a>
								</td>
								<td class="csui" width="1%">
									<img src="/cs/images/icons/controls/black/delete.png" border="0" style="cursor:pointer" title="Delete Library All groups" onclick="deletefield(<%=r.getString("ID") %>);" >
								</td>
								
								<td class="csui" width="1%">
									
									<img src="/cs/images/icons/controls/black/delete.png" border="0" style="cursor:pointer" title="Delete Library Reference from this group" onclick="deletetype(<%=r.getString("REF_LIBRARY_GROUP_ID") %>,<%=r.getString("ID") %>);" >
