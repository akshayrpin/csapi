...
<%
	...
	//Pagination
	int maxrows = map.getInt("MAX", 5000);
	int pg = map.getInt("PAGE",1);
	String sortfield= map.getString("SORT_FIELD","TITLE");
	...
	function showselectorui(id,output){
			...
			//c += '<table class="sortable">';
	 		...
	 		$.each(output, function(k,v) {
			 		...
			 		var u = "clist.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>&process=edit&ID=";
			 		u +=v.ID; 
			 		u +="&groupId="+id; 
			 		u +="&libraryId="+v.ID; 
			 		u +="&REQ="+v.REQ; 
			 		u +="&ORDR="+v.ORDR; 
			 		var associatecolor = "black"; 	if(v.MULTI>1){ 	associatecolor ="color";	}
					...
					$.ajax({
			   			  ...
			   			  data: { 
			   				  ...
			   				  _id : <%=id%>,
			   				   ...
			   				   connectids : "<%=connectids%>",
			   				   //REF_ID : id,
			   				   LIBRARY_GROUP_ID : fieldgroupsid
			   			      //mode : mode
			   			    },
			   			    success: function(output) {
	...
	<div id="csuibody">
		<div id="csuimain">
			...
			<br/><br/>
				...
				<input type="hidden" name="SEARCH_FIELDS" value="TITLE,TXT,LIBRARY_CODE">
				...
				<div class="csui_divider"></div>
				   ...
				   <table class="csui_title" alert="warning">
						...
						<tr>
							...
							<td class="csui_header">REQUIRED</td>
						...
						<%for(int i=0;i<a.size();i++){
							...
							String u = "clist.jsp?_ent=permit&_id="+id+"&_type="+type+"&_typeid="+typeid+"&process=edit&ID="+r.getString("ID")+"&libraryId="+r.getString("ID")+"&ORDR=0";
