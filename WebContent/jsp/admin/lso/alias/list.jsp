...
<%
	...
	MapSet m = AdminMap.getLsoLst(map,table,start,end,sortfield,order,map.getString("SQ"),map.getString("SEARCH_FIELDS"));
		...
		$(document).ready(function() {
				...
				<%if(Operator.hasValue(connectids)){ %>
				 ...
				 <%for(int i=0;i<c.size();i++){
					...
					%>
					var id = "<%= r.getString("LKUP_MODULE_ID")%>";
					$('#tr_'+id).hide();
				...
				<%if(c.size()>0){ %>
				 ...
				 $('#refs2').sortable({
						update: function saveOrder(){
							var result = $('#refs2').sortable('toArray');
							var order ="";
							for (var i = 0; i < result.length; i++) {
								order += result[i] + ",";
							}
	...
	<div id="csuibody">
		<div id="csuimain">
			...
			<br/><br/>
				...
				<table align="right">
					...
					<tr>
						<td class="search"  type="String" itype="String" alert="" align="right">
							<select  name="LOCATION" id="LOCATION" itype="String" val="" _ent="lso" valrequired="true" >
				...
				<div class="csui_divider"></div>
				   ...
				   <table class="csui_title" alert="warning">
						...
						<tr>
							...
							-->
							<td class="csui_header">STREET NUMBER</td>
							<td class="csui_header">STREET NAME</td>
							<td class="csui_header">CITY</td>
							<td class="csui_header">SATE</td>
							<td class="csui_header">ZIP</td>
							<td class="csui_header">DESCRIPTION</td>
							<td class="csui_header">ACTIVE</td>
						 ...
						 <tbody>
						<%for(int i=0;i<lsoList.size();i++){
							MapSet r = lsoList.get(i);
						%>
							<tr id="tr_<%=r.getString("ID") %>">
								...
								<%} %>
								<td class="csui" style="cursor:pointer;" rel="<%=r.getString("ID") %>" ><%=r.getString("STR_NO") %>
								 <input type="hidden" id="refname_<%=r.getString("ID") %>" name="refname_<%=r.getString("ID") %>" value="<%=r.getString("MODULE") %>">
								 
								</td>
								<td class="csui"  style="cursor:pointer;" rel="<%=r.getString("ID") %>" ><%=r.getString("STREET_NAME") %></td>
								<td class="csui"  style="cursor:pointer;" rel="<%=r.getString("ID") %>" ><%=r.getString("CITY") %></td>
								<td class="csui"  style="cursor:pointer;" rel="<%=r.getString("ID") %>" ><%=r.getString("STATE") %></td>
								<td class="csui"  style="cursor:pointer;" rel="<%=r.getString("ID") %>" ><%=r.getString("ZIP") %></td>
								<td class="csui"  style="cursor:pointer;" rel="<%=r.getString("ID") %>" ><%=r.getString("DESCRIPTION") %></td>
								<td class="csui"  style="cursor:pointer;" rel="<%=r.getString("ID") %>" ><%=r.getString("ACTIVE") %></td>								
		...
		<div id="csuisub">
				...
				<div class="csuisubcontent">
					<%if(c.size()>0 || Operator.hasValue(connectids)){ %>
				<table class="csuisub_title  " alert="warning"  cellpadding="20" cellspacing="0" border="0" type="horizontal" >
						<tr>
							<td class="csui">
								...
								<%for(int i=0;i<c.size();i++){
								 	MapSet r = c.get(i);
								 	if(r.getString("LOCATION").equalsIgnoreCase("summary")){
								%>
				...
				<table class="csuisub_title  " alert="warning"  cellpadding="20" cellspacing="0" border="0" type="horizontal" >
						...
						<tr>
							<td class="csui">
								...
								<%for(int i=0;i<c.size();i++){
								 	MapSet r = c.get(i);
								 	if(r.getString("LOCATION").equalsIgnoreCase("info")){
								%>
								
									<table class="csui" type="horizontal" id="cr_<%=r.getString("LKUP_MODULE_ID")%>" style="cursor:pointer;" title="Drag to sort">
										<tr>
											<td class="csui" width="1%">
												<img src="/cs/images/icons/controls/black/delete.png" border="0" onclick="deleteref(<%=r.getString("LKUP_MODULE_ID") %>);" >
											</td>
											<td class="csui">
												<%=r.getString("MODULE")%>
											</td>
