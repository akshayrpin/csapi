...
<%
	...
	if(connector){
		MapSet m = AdminMap.getTemplateRef(map);
		...
		$(document).ready(function() {
				...
				<%if(Operator.hasValue(connectids)){ %>
				 ...
				 <%for(int i=0;i<c.size();i++){
					...
					%>
					var id = "<%= r.getString("TEMPLATE_ID")%>";
					$('#tr_'+id).hide();
			...
			<table cellpadding="0" cellspacing="0" border="0" width="100%">
				<tr>
					...
					<table class="csui_tools">
 					 <tr>
						<td class="csui_tools">
						<a target="_self" href="add.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>" ><img src="/cs/images/icons/controls/white/add.png" border="0"></a>
						</td>
						...
						<tr>
							<td class="csui">
								...
								%>
										...
										<tr  class="csui" id="cr_<%=r.getString("TEMPLATE_ID")%>" style="cursor:pointer;" title="Drag to sort" >
											<td class="csui" width="1%">
												<img src="/cs/images/icons/controls/black/delete.png" border="0" onclick="deleteref(<%=r.getString("TEMPLATE_ID") %>);" >
											</td>
											<td class="csui">
												<%=r.getString("NAME")%> 
											</td>
											<td class="csui">
												<input id="ONLINE" name="ONLINE" type="checkbox" class="ajauto" rel="<%=r.getString("ID")%>" <%if(r.getString("ONLINE").equals("Y")){ %>checked <% }%> >
											</td>
										</tr>
