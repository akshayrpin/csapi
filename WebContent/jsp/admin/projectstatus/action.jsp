...
<%
	...
	if(Operator.equalsIgnoreCase(action, "addref")){
		...
		int groupid = map.getInt("LKUP_PROJECT_STATUS_ID");
		
		if(groupid>0 && Operator.hasValue(connecttype)){
			MapSet m = AdminMap.getProjectStatusRef(map);
			System.out.println(groupid+"######INSIDE HERE######"+m.getString("LKUP_PROJECT_TYPE_ID"));
			if(m.getString("DEFLT").equals("Y")){
				  AdminAgent.saveType("UPDATE REF_PROJECT_TYPE_STATUS SET DEFLT ='N' WHERE LKUP_PROJECT_TYPE_ID IN ("+m.getString("connectIds")+")");
			 }
