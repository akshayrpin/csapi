...
<%
	...
	if(connector){
		MapSet m = AdminMap.getProjectStatusRef(map);
			 ...
			 $('#multiedit').click(function () {
			  ...
			  $('#merge').click(function () {
				 ...
				 <%for(int i=0;i<c.size();i++){
					...
					%>
					var id = "<%= r.getString("LKUP_PROJECT_STATUS_ID")%>";
					$('#tr_'+id).hide();
...
<div id="loader"></div>
  ...
  <tr>
  	<td class="csui_tools">
		<a href="<%=Config.fullcontexturl()%>/jsp/admin/projectstatus/merge.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>" target="lightbox-iframe" loc="<%=Config.fullcontexturl()%>/jsp/admin/projectstatus/merge.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>" id="merge" title="Merge" border="0"  ><img src="/cs/images/icons/merge.png" border="0"></a>
	</td>
	<td class="csui_tools">
	...
	<td class="csui_tools">
	  <a href="<%=Config.fullcontexturl()%>/jsp/admin/projectstatus/addmulti.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>" target="lightbox-iframe" loc="<%=Config.fullcontexturl()%>/jsp/admin/projectstatus/addmulti.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>" id="multiedit" title="Multi Edit" border="0"  ><img src="/cs/images/icons/multi-edit.png" border="0"></a>
	</td>
					 ...
					 <thead>
						...
						%>
							...
							<%if(!connector){ %>
								...
								<%} else { %>
									<td class="csui" colnum="2" type="boolean" itype="boolean" alert="" ><div><input name="DEFLT_<%=r.getString("ID") %>" id="DEFLT_<%=r.getString("ID") %>" type="checkbox"  itype="boolean" ></div></td>
									<td class="csui"><%=r.getString("C_CREATED_DATE") %></td>
						...
						<tr>
							<td class="csui">
								...
								%>
									...
									<table class="csui" type="horizontal" id="cr_<%=r.getString("LKUP_PROJECT_STATUS_ID")%>" style="cursor:pointer;" title="Drag to sort">
										<tr>
											<td class="csui" width="1%">
												<img src="/cs/images/icons/controls/black/delete.png" border="0" onclick="deleteref(<%=r.getString("LKUP_PROJECT_STATUS_ID") %>);" >
											</td>
											<td class="csui">
												<%=r.getString("NAME")%>- <%=r.getString("DEFLT")%>
											</td>
