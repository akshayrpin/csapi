...
<%
	...
	if(Operator.equalsIgnoreCase(action, "showselector")){
		...
		if(groupid>0){
			...
			JSONArray o = AdminAgent.getJsonList(AdminMap.getReviewFields(groupid));
			//o.put(1,);
	...
	if(Operator.equalsIgnoreCase(action, "addref")){
		...
		int groupid = map.getInt("REVIEW_GROUP_ID");
	...
	if(Operator.equalsIgnoreCase(action, "sortfields")){
		int REVIEW_GROUP_ID = map.getInt("REVIEW_GROUP_ID");
		if(REVIEW_GROUP_ID>0){
			MapSet m = AdminMap.getReviews(map);
			boolean r = AdminAgent.sortRef(m);
			out.write(r+"");
		}
	}
