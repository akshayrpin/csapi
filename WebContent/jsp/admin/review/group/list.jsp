...
<%
	...
	if(connector){
		MapSet m = AdminMap.getReviewGroupRef(map);
...
<head>
			 ...
			 $('#multiedit').click(function () {
				...
				<%if(Operator.hasValue(connectids)){ %>
				 ...
				 <%for(int i=0;i<c.size();i++){
					...
					%>
					var id = "<%= r.getString("REVIEW_GROUP_ID")%>";
					$('#tr_'+id).hide();
	...
	function showselectorui(id,output){
			...
			c += '<td class="csui_header pymnt" width="1%" nowrap>DISPLAY TYPE</td>';
			...
			//c += '<table class="sortable">';
	 		...
	 		$.each(output, function(k,v) {
			 		...
			 		c += '<td class="csui pymnt" type="String" itype="String">'+v.DISPLAY_TYPE+'</td>';
				...
				<tr>
					...
					<table class="csui_tools">
 					 <tr>
						...
						<td class="csui_tools">
	                      <a href="<%=Config.fullcontexturl()%>/jsp/admin/review/group/addmulti.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>" target="lightbox-iframe" loc="<%=Config.fullcontexturl()%>/jsp/admin/review/group/addmulti.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>" id="multiedit" title="Multi Edit" border="0"  ><img src="/cs/images/icons/multi-edit.png" border="0"></a>
                        </td>
				   ...
				   <table class="csui_title" alert="warning">
						...
						%>
							<tr id="tr_<%=r.getString("ID") %>">
								...
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("AUTO_ADD") %></td>
						...
						<tr>
							<td class="csui">
								...
								%>
									...
									<table class="csui" type="horizontal" id="cr_<%=r.getString("REVIEW_GROUP_ID")%>" style="cursor:pointer;" title="Drag to sort">
										<tr>
											<td class="csui" width="1%">
												<img src="/cs/images/icons/controls/black/delete.png" border="0" onclick="deleteref(<%=r.getString("REVIEW_GROUP_ID") %>);" >
											</td>
											<td class="csui">
												<%=r.getString("GROUP_NAME")%> : <%=r.getString("GROUP_DESCRIPTION")%>
											</td>
