...
<%
	...
	String table = Table.LKUPJOBSCHEDULETABLE;
...
<head>
	...
	function indexSolr(id ,indexUrl,packageName) 
	{
		var i=0;
		var method = "indexSolr";
		for (i = 0; i <= 10; i++) {
		 $("#index"+i).hide();  
		 $("#bt_loading"+i).show();
	...
	$.ajax({
		      type:"POST",
		      url: "<%=Config.fullcontexturl()%>/jsp/admin/solr/action.jsp?_action="+method,
		      dataType: 'text',	   	  
   			  data: { 
   				// feesjson : type,
   			      indexUrl : indexUrl,
   			   packageName : packageName,
   			    ...
   			    success: function(output) {
   			    		...
   			    		swal("Indexing completed!", packageName+" Indexing is completed.", "success");   
				...
				<div class="csui_divider"></div>
				   ...
				   <table class="csui_title" alert="warning">
						...
						<tr>
							...
							-->
							<td class="csui_header">SOLR JOB NAME</td>
							<td class="csui_header">SOLR JOB URL</td>
						 ...
						 <tbody>
						<%
						int count = AdminAgent.getActiveJobCount();
						...
						%>
							<tr id="tr_<%=r.getString("ID") %>">
								...
								<td class="csui"  style="cursor:pointer;" id="index<%=i%>" rel="<%=r.getString("ID") %>" >
								
								<!-- <a href="" target="_new"> -->
								
								<input type="button" name="_index" value="Index" class="csui_button" onclick="indexSolr('<%=r.getString("ID") %>' ,'<%=r.getString("JOB_URL") %>','<%=r.getString("JOB_URL") %>')">
								<!-- </a> -->
								</td>  
								
								<td class="csui" style="display:none;cursor:pointer;" id="bt_loading<%=i%>" rel="<%=r.getString("ID") %>" >
								
								<input type="image" src="/cs/images/loading.gif" alt="Indexing <%=r.getString("DESCRIPTION") %>" title="Indexing <%=r.getString("DESCRIPTION") %>" />
