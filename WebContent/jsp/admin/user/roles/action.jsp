...
<%
	...
	if(Operator.equalsIgnoreCase(action, "addref")){
		...
		int groupid = map.getInt("LKUP_ROLES_ID");
	...
	if (map.equalsIgnoreCase("_action", "associate")) {
		MapSet m = AdminMap.insertUserRolesRef(map);
		String table = Table.REFUSERROLESTABLE;
		boolean resp = AdminAgent.saveTypeMultiValues(m, table);
		System.out.println("copy response "+resp);
		out.print(resp);
	}
	
	if (map.equalsIgnoreCase("_action", "associatetypes")) {
		MapSet m = AdminMap.insertRolesRef(map);
		String table = m.getString("reftable");
		boolean resp = AdminAgent.saveTypeMultiValues(m, table);
		System.out.println("associate response "+resp);
		out.print(resp);
	}
	
	if (map.equalsIgnoreCase("_action", "deleterefroles")) {
		MapSet m = AdminMap.getCommon(map);
		String table =map.getString("ref");
		m.add("ID",map.getInt("ID"));
		m.add("ACTIVE","N");
		boolean resp = AdminAgent.updateType(m, table);
		out.print(resp);
	...
	if (map.equalsIgnoreCase("_action", "updateflags")) {
		...
		String reffeegrouptable = map.getString("tb");
