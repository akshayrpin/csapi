...
<%
	...
	String sortfield= map.getString("SORT_FIELD","ROLE");
	...
	if(connector){
		MapSet m = AdminMap.getUserRef(map);
...
<head>
		...
		$(document).ready(function() {
				...
				<%if(Operator.hasValue(connectids)){ %>
				 ...
				 <%for(int i=0;i<c.size();i++){
					...
					%>
					var id = "<%= r.getString("LKUP_ROLES_ID")%>";
					$('#tr_'+id).hide();
	...
	function showselectorui(id,output){
			...
			c += '<td class="csui_header pymnt" width="1%" nowrap>TITLE</td>';
			c += '<td class="csui_header pymnt" width="1%" nowrap>CODE</td>';
			c += '<td class="csui_header pymnt" width="1%" nowrap>INSPECTABLE</td>';
			c += '<td class="csui_header pymnt" width="1%" nowrap>WARNING</td>';
			c += '<td class="csui_header pymnt" width="1%" nowrap>COMPLETE</td>';
			c += '<td class="csui_header pymnt" width="1%" nowrap>REQUIRED</td>';
			c += '<td class="csui_header pymnt" width="1%" nowrap>LIBRARY DESC</td>';
			...
			//c += '<table class="sortable">';
	 		...
	 		$.each(output, function(k,v) {
			 		...
			 		c += '<td class="csui pymnt" type="String"  itype="String" width="1%" nowrap>'+v.TITLE+'</td>';
			 		c += '<td class="csui pymnt" type="String" itype="String" width="1%" nowrap>'+v.LIBRARY_CODE+'</td>';
			 		c += '<td class="csui pymnt" type="String" itype="String">'+v.INSPECTABLE+'</td>';
			 		c += '<td class="csui pymnt" type="String" itype="String">'+v.WARNING+'</td>';
			 		c += '<td class="csui pymnt" type="String" itype="String">'+v.COMPLETE+'</td>';
			 		c += '<td class="csui pymnt" type="String" itype="String">'+v.REQUIRED+'</td>';
			 		c += '<td class="csui pymnt" type="String" itype="String">'+v.LIBRARY_DESC+'</td>';
	...
	<div id="csuibody">
		<div id="csuimain">
			...
			<br/><br/>
				...
				<input type="hidden" name="SEARCH_FIELDS" value="ROLE">
				...
				<div class="csui_divider"></div>
				   ...
				   <table class="csui_title" alert="warning">
						...
						<tr>
							...
							<td class="csui_header">EVERYONE</td>
							<td class="csui_header">STAFF</td>
							<td class="csui_header">PEOPLE</td>
						...
						%>
							<tr id="tr_<%=r.getString("ID") %>">
								...
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("ADMIN") %></td>
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("EVERYONE") %></td>
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("STAFF") %></td>
								<td class="csui" style="cursor:pointer;"  rel="<%=r.getString("ID") %>" ><%=r.getString("PEOPLE") %></td>
								...
								<td class="csui" width="1%">
									<a target="_self" href="add.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>&ID=<%=r.getString("ID") %>&process=edit" ><img src="/cs/images/icons/controls/black/edit.png" border="0"></a>
								</td>
								<td class="csui" width="1%">
						...
						<tr>
							<td class="csui">
								...
								%>
									...
									<table class="csui" type="horizontal" id="cr_<%=r.getString("LKUP_ROLES_ID")%>" style="cursor:pointer;" title="Drag to sort">
										<tr>
											<td class="csui" width="1%">
												<img src="/cs/images/icons/controls/black/delete.png" border="0" onclick="deleteref(<%=r.getString("LKUP_ROLES_ID") %>);" >
											</td>
											<td class="csui">
												<%=r.getString("ROLE")%>
											</td>
