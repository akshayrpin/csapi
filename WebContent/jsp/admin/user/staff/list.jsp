...
<%
	...
	String sortfield= map.getString("SORT_FIELD","DEPARTMENT_ID");
	...
	ArrayList<MapSet> sla = AdminAgent.getSubIndex(type, typeid, id);
	 ...
	 if(map.equalsIgnoreCase("SEARCH","SEARCH")){
		a =  AdminAgent.getListWithAdditional(table,start,end,sortfield,order,map.getString("SQ"),map.getString("SEARCH_FIELDS"), " ,u.FIRST_NAME,u.LAST_NAME,u.MI,u.USERNAME,u.EMAIL, d.DEPT", "join users u on a.users_id = u.id join department d on a.department_id= d.id ", "");
		//records =5;
	...
	//	urlsort.append("/admin/formmgr/formEntries.jsp?SEARCH=SEARCH&FORM_ID=").append(formId).append("&CATEGORY_ID=").append(categoryId).append("&MAX=").append(maxrows).append("&Q=").append(q).append("&searchfields=").append(searchfields).append("&displayfields=").append(displayfields);
		...
		a =  AdminAgent.getListWithAdditional(table, start, end, sortfield, order, "", "", ", u.FIRST_NAME,u.LAST_NAME,u.MI,u.USERNAME,u.EMAIL, d.DEPT", "join users u on a.users_id = u.id join department d on a.department_id= d.id ", "");
		//records =11;
	...
	<div id="csuibody">
		<div id="csuimain">
			...
			<br/><br/>
				...
				<input type="hidden" name="SEARCH_FIELDS" value="u.USERNAME,u.FIRST_NAME,u.EMAIL,u.LAST_NAME,d.dept,phone_work,phone_cell,title">
					...
					<table class="csui tablesorter" type="horizontal">
					 <thead>
						<tr>
							...
							-->
							<td class="csui_header">USERNAME</td>
							<td class="csui_header">EMPL NUM</td>
						
							
							
						
							<td class="csui_header">EMAIL</td>
							<td class="csui_header">DEPT</td>
							<td class="csui_header">FIRST NAME</td>
							<td class="csui_header">MI</td>
							<td class="csui_header">LAST NAME</td>
							<td class="csui_header">TITLE</td>
							<td class="csui_header">PHONE WORK</td>
							<td class="csui_header">PHONE CELL</td>
						...
						%>
							<tr id="tr_<%=r.getString("ID") %>">
								<td class="csui"><input type="checkbox" name="selector" id="selector" class="selector" value="<%=r.getString("USERS_ID") %>"> </td>
								<td class="csui"><%=r.getString("USERNAME") %></td>
								<td class="csui"><%=r.getString("EMPL_NUM") %></td>
								<td class="csui"><%=r.getString("EMAIL") %></td>
								<td class="csui"><%=r.getString("DEPT") %></td>
								<td class="csui"><%=r.getString("FIRST_NAME") %></td>
								<td class="csui"><%=r.getString("MI") %></td>
								<td class="csui"><%=r.getString("LAST_NAME") %></td>
								<td class="csui"><%=r.getString("TITLE") %></td>
								<td class="csui"><%=r.getString("PHONE_WORK") %></td>
								<td class="csui"><%=r.getString("PHONE_CELL") %></td>
								
						
								
								<td class="csui" width="1%">
								<a target="lightbox-iframe" href="../user/user.jsp?_ent=<%=ent%>&_id=<%=id%>&_type=<%=type%>&_typeid=<%=typeid%>&ID=<%=r.getString("USERS_ID") %>&process=edit" ><img src="/cs/images/icons/controls/black/edit.png" border="0"></a>
								</td>
