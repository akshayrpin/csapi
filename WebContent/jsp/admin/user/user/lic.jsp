...
<%
	...
	String table = Table.PEOPLELICENSETABLE;
	String alert="";
	...
	boolean result = false;
	 if(map.equalsIgnoreCase("_action","SAVE")){
		 if(editId>0){
			 result = AdminAgent.updateType(AdminMap.getLicenseType(map),table);
		 }else {
			 result = AdminAgent.saveType(AdminMap.getLicenseType(map),table);
		 }
	 ...
	 JSONArray lkuplic = AdminAgent.getLookup("LKUP_PEOPLE_LICENSE_TYPE");
	...
	<script>
		$(document).ready(function() {
			...
			<% if(editId>0){%>
				...
				$('#LKUP_PEOPLE_LICENSE_TYPE_ID').val(<%=v.getString("LKUP_PEOPLE_LICENSE_TYPE_ID")%>);
				$('#LKUP_PEOPLE_LICENSE_TYPE_ID').trigger('chosen:updated');
			<% }%>
...
<div id="csuibody">
		<div id="csuimain">
			...
			<div class="csuicontent">
				...
				<form id="csform" action="lic.jsp" method="post">
					<input type="hidden" name="_type" value="<%=type%>">
					<input type="hidden" name="_typeid" value="<%=typeid%>">
					<input type="hidden" name="_id" value="<%=id%>">
					<input type="hidden" name="ID" value="<%=editId%>">
					<input type="hidden" name="tabletype" value="single">
					<input type="hidden" name="PEOPLE_ID" value="<%=map.getInt("PEOPLE_ID")%>">
					
					<table class="csui_title">
						<tr>
							<td class="csui_title">PEOPLE LICENSE</td>
						...
						<table class="csui" colnum="2" type="default">
							...
							<tr>
									<td class="csui_label" colnum="2" alert="">LICENSE TYPE</td>
									<td class="csui" colnum="2" type="String" itype="text" alert="" colspan="4">
									<select  name="LKUP_PEOPLE_LICENSE_TYPE_ID" id="LKUP_PEOPLE_LICENSE_TYPE_ID" itype="String" val="" _ent="lso" valrequired="true"><option value=""></option>
										<%for(int i=0;i<lkuplic.length();i++){ %>	<option value="<%=lkuplic.getJSONObject(i).getInt("ID")%>"><%=lkuplic.getJSONObject(i).getString("TEXT")%></option>		<%}%>
									</select>
									
									</td>
								</tr>
							<tr>
								<td class="csui_label" colnum="2" alert="">LIC_NUM</td>
								<td class="csui" colnum="2" type="String" itype="text" alert=""><input name="LIC_NUM" type="text" itype="text" value="<%=v.getString("LIC_NUM") %>" valrequired="true" maxchar="10000"></td>
								<td class="csui_label" colnum="2" alert="">LIC_EXPIRATION_DATE</td>
								<td class="csui" colnum="2" type="String" itype="date" alert=""><input name="LIC_EXPIRATION_DATE" type="text" itype="date" value="<%=v.getString("LIC_EXPIRATION_DATE") %>" valrequired="true" maxchar="10000"></td>
							</tr>
